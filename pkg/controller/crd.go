...
func EnsureCRDCreated(ctx context.Context, client apiextensionsclient.Interface) (created bool, err error) {
	crdClient := client.ApiextensionsV1beta1().CustomResourceDefinitions()
	presetCRD, err := crdClient.Get(ctx, CRD.Name, metav1.GetOptions{})
	...
	if reflect.DeepEqual(presetCRD.Spec, CRD.Spec) {
		...
		// Update CRD
		if _, err := crdClient.Update(ctx, newCRD, metav1.UpdateOptions{}); err != nil {
